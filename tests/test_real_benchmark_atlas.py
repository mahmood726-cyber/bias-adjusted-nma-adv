...
def test_real_benchmark_atlas_summarizes_current_source_backed_coverage():
    ...
    assert atlas["n_benchmarks"] == 36
    assert atlas["n_benchmark_study_effects"] == 201
    assert atlas["n_tau2_positive_benchmarks"] == 22
    assert atlas["n_unique_study_ids"] == 178
    assert atlas["n_unique_nct_ids"] == 143
    assert atlas["n_unique_pmids"] == 138
    ...
    assert atlas["evidence_mode_counts"] == {
        ...
        "reported_hr_pubmed_abstract": 29,
    ...
    benchmark_ids = {item["id"] for item in atlas["benchmarks"]}
    assert len(benchmark_ids) == atlas["n_benchmarks"]
    assert {
    ...
    assert len(positive_tau_ids) == atlas["n_tau2_positive_benchmarks"]
    assert {
        "sglt2_ckd_reported_hr",
        "glp1_mace_reported_hr",
        "parp_firstline_ovarian_pfs_reported_hr",
...
def test_real_benchmark_atlas_summary_is_validation_status_ready():
    ...
    assert summarize_real_benchmark_atlas(atlas) == {
        ...
        "status": "passed",
        "n_benchmarks": 36,
        "n_benchmark_study_effects": 201,
        "n_tau2_positive_benchmarks": 22,
        "n_unique_nct_ids": 143,
        "n_unique_pmids": 138,
        "domain_counts": {
            "arm_count_binary_closed_loop_network": 1,
            "binary_pairwise_meta": 1,
