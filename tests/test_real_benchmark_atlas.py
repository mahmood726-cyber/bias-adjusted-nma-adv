...
def test_real_benchmark_atlas_summarizes_current_source_backed_coverage():
    ...
    assert atlas["n_benchmarks"] == 10
    assert atlas["n_benchmark_study_effects"] == 76
    assert atlas["n_tau2_positive_benchmarks"] == 1
    assert atlas["n_unique_study_ids"] == 59
    assert atlas["n_unique_nct_ids"] == 23
    assert atlas["n_unique_pmids"] == 15
    ...
    assert atlas["source_check_scope_counts"] == {
        ...
        "pubmed_abstract_reported_hr_tokens": 9,
    }
