"""Verify a source-backed dose-response manifest against CT.gov and PubMed."""
...
DEFAULT_MANIFEST = Path("validation/dose_response/semaglutide_obesity_dose_response.toml")
DEFAULT_OUTPUT = Path("validation/source_checks/semaglutide_obesity_dose_response_check.json")


def fetch_bytes(url: str, timeout: int) -> tuple[int, bytes]:
    request = Request(url, headers={"User-Agent": "bias-adjusted-nma-adv-dose-response-verifier/0.1"})
    try:
...
def verify_ctgov(manifest: dict[str, object], timeout: int) -> dict[str, object]:
                ...
                arm_checks.append(
                    {
                        ...
                        "se_found": number_equal(measurement.get("spread", ""), arm["se"]),
                        "source_terms_found": contains_all(group_text, [str(arm["source_terms"][0])]),
                    }
                )
            details["arm_checks"] = arm_checks
            details["arm_values_found"] = all(
                item["value_found"] and item["se_found"] for item in arm_checks
            )
    ...
    return {
        ...
        "identifier": nct_id,
        "evidence_scope": "clinicaltrials_gov_dose_response_lsmean",
...
def verify_pubmed(manifest: dict[str, object], timeout: int) -> dict[str, object]:
    ...
    details: dict[str, object] = {
        ...
        "dose_ranging_terms_found": False,
    }
    if status == 200:
        parsed = json.loads(payload.decode("utf-8"))
        ...
        details["title_terms_found"] = contains_all(title, ["semaglutide", "liraglutide", "placebo"])
        details["dose_ranging_terms_found"] = contains_all(title, ["dose", "ranging"])
    verified = all(bool(details[key]) for key in ("pmid_found", "title_terms_found", "dose_ranging_terms_found"))
    return {
        "source_type": "pubmed_abstract",
        "identifier": pmid,
        "evidence_scope": "pubmed_abstract_dose_response_identity",
        "response_sha256": sha256_bytes(payload),
        "verified": verified,
        "details": details,
    }
...
def build_report(manifest_path: Path, checked_at: str, timeout: int, pause_seconds: float) -> dict[str, object]:
    ...
    return {
        "schema_version": "dose_response_source_verification/v1",
