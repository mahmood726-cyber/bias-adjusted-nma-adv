"""Write the source-verified component-NMA smoke benchmark artifact."""
...
from bias_nma_adv.component_benchmark import run_component_benchmark


DEFAULT_MANIFEST = Path("validation/component/sitagliptin_pioglitazone_component.toml")
DEFAULT_SOURCE_CHECK = Path("validation/source_checks/sitagliptin_pioglitazone_component_check.json")
DEFAULT_OUTPUT = Path("validation/component/sitagliptin_pioglitazone_component_source_benchmark.toml")
...
def artifact_to_toml(artifact: dict[str, Any]) -> str:
    ...
    fit_without_rows = {
        key: value
        for key, value in artifact["candidate"]["additive_component_wls"].items()
        if key != "component_effects"
    }
    append_table(lines, "candidate.additive_component_wls", fit_without_rows)
    append_array_table(
        lines,
        "candidate.additive_component_wls.component_effects",
        artifact["candidate"]["additive_component_wls"]["component_effects"],
    )
    lines.append("")
    return "\n".join(lines)
...
def main() -> int:
    ...
    artifact = run_component_benchmark(args.manifest, verification_report_path=args.source_check)
