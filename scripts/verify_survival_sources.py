"""Verify survival HR manifest identities against public CT.gov/PubMed APIs."""
...
DEFAULT_OUTPUT = Path("validation/source_checks/sglt2_hf_reported_hr_source_check.json")


def fetch_bytes(url: str, timeout: int) -> tuple[int, bytes]:
    request = Request(url, headers={"User-Agent": "bias-adjusted-nma-adv-survival-source-verifier/0.1"})
    try:
...
def verify_ctgov(study: dict[str, object], timeout: int) -> dict[str, object]:
    ...
    return {
        ...
        "manifest_url": f"https://clinicaltrials.gov/study/{identifier}",
        "api_url": api_url,
        "http_status": status,
...
def verify_pubmed(study: dict[str, object], timeout: int) -> dict[str, object]:
    ...
    return {
        ...
        "identifier": identifier,
        "manifest_url": str(study["source_url"]),
        "api_url": api_url,
        "http_status": status,
        "identity_verified": identity_verified,
        "response_sha256": sha256_bytes(payload),
