"""Write the source-verified CT.gov reported-HR network benchmark artifact."""
...
from bias_nma_adv.ctgov_hr_network import run_ctgov_hr_network_benchmark


DEFAULT_MANIFEST = Path("validation/networks/t2d_mace_ctgov_hrs.toml")
DEFAULT_SOURCE_CHECK = Path("validation/source_checks/t2d_mace_ctgov_hr_network_check.json")
DEFAULT_OUTPUT = Path("validation/networks/t2d_mace_ctgov_hr_network_benchmark.toml")
...
def artifact_to_toml(artifact: dict[str, Any]) -> str:
    ...
    for model_key, fit in artifact["candidate"].items():
...
def main() -> int:
    ...
    artifact = run_ctgov_hr_network_benchmark(
        args.manifest,
