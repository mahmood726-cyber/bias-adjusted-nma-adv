...
Validated local R output: `validation/reference_runs/pairwise_metafor_meta_reference.toml` validates `validation/reference_runs/pairwise_metafor_meta_output.json` against this source-backed benchmark. It matches study-level log-OR effects and the fixed-effect pooled result within the stated tolerance, and it records the expected HKSJ difference: `metafor` uses the unfloored KNHA factor here, while this Python artifact applies the prespecified HKSJ floor. `validation/reference_runs/sglt2_hf_metafor_gosh_reference.toml` also validates `validation/reference_runs/sglt2_hf_metafor_gosh_output.json` against `metafor::gosh` for all non-empty fixed-effect subsets of the same source-backed study rows. `validation/reference_runs/psoriasis_sparse_binary_metafor_reference.toml` validates a separate CT.gov/PubMed low-control-event psoriasis binary count contrast against `metafor::rma.uni`, while preserving zero-event and Mantel-Haenszel limitations. `validation/reference_runs/breast_adjuvant_idfs_prediction_interval_metafor_reference.toml` validates a source-backed reported-HR prediction-interval convention against `metafor::predict` KNHA while separately preserving the local HKSJ floor disclosure. These are narrow evidence-candidate parity artifacts, not broad production certification.
...
## Benchmark 12: Inclisiran ORION Continuous LDL-C Change

Continuous CT.gov effects CSV: `validation/continuous/inclisiran_orion_ldlc_pct_ctgov_effects.csv`

Continuous source snapshot: `validation/source_checks/inclisiran_orion_ldlc_pct_ctgov_check.json`

Continuous benchmark artifact: `validation/continuous/inclisiran_orion_ldlc_pct_ctgov_benchmark.toml`

External `metafor`/`meta` reference output: `validation/reference_runs/inclisiran_orion_continuous_metafor_reference.toml` validates `validation/reference_runs/inclisiran_orion_continuous_metafor_output.json`.

Trials: ORION-9 (`NCT03397121`, PMID 32197277), ORION-10 (`NCT03399370`, PMID 32187462), and ORION-11 (`NCT03400800`, PMID 32187462).

Outcome: CT.gov-reported adjusted treatment difference for percent LDL-C change at day 510, inclisiran versus placebo.
...
- ORION rows share a related drug program and endpoint family, so they broaden continuous-outcome software validation but do not prove broad domain coverage;
- the benchmark uses CT.gov adjusted treatment differences as reported and does not extract full open-access paper tables;
- this adds a second independent continuous domain to the `metafor`/`meta` candidate set, but broad pairwise continuous-outcome parity remains blocked.
...
| External pairwise reference output | Dynamic local R execution plus Python validation | `validation/reference_runs/pairwise_metafor_meta_output.json`, `validation/reference_runs/pairwise_metafor_meta_reference.toml`, `validation/reference_runs/psoriasis_sparse_binary_metafor_reference.toml`, `validation/reference_runs/breast_adjuvant_idfs_prediction_interval_metafor_reference.toml`, and `src/bias_nma_adv/r_reference_validation.py` | Validates narrow `metafor`/`meta` output parity for study log-OR effects, fixed-effect pooling, REML tau2/Q/df, documented HKSJ floor differences, one CT.gov/PubMed low-control-event binary contrast, and one reported-HR prediction-interval convention; not production certification |
...
| External pairwise tau2 cross-check reference output | Dynamic local R execution plus Python validation | `external/r/metafor_tau2_crosscheck_survival.R`, `validation/reference_runs/metafor_tau2_crosscheck_survival_output.json`, `validation/reference_runs/metafor_tau2_crosscheck_survival_reference.toml`, and `src/bias_nma_adv/r_reference_validation.py` | Validates FE/DL/PM/REML pairwise summaries across four source-backed reported-HR benchmarks; not broad optimizer stress parity or production certification |
