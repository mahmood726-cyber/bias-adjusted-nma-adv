...
| External pairwise reference output | Dynamic local R execution plus Python validation | `validation/reference_runs/pairwise_metafor_meta_output.json`, `validation/reference_runs/pairwise_metafor_meta_reference.toml`, `validation/reference_runs/psoriasis_sparse_binary_metafor_reference.toml`, and `src/bias_nma_adv/r_reference_validation.py` | Validates narrow `metafor`/`meta` output parity for study log-OR effects, fixed-effect pooling, REML tau2/Q/df, documented HKSJ floor differences, and one CT.gov/PubMed low-control-event binary contrast; not production certification |
