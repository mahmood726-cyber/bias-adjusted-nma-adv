...
Current local multi-arm diagnostic capability: `fit_multiarm_gls(...)` returns row-level influence diagnostics for each fitted contrast row: observed value, fitted value, residual, GLS leverage, standardized residual, and an approximate Cook distance. These diagnostics help identify high-leverage or discordant contrasts in the deterministic fixture tests, but they do not yet replace `netmeta`'s wider diagnostic ecosystem and do not resolve the feature-completeness blocker.
