...
Run `python scripts/validate_benchmark_registry.py --json` to execute this gate outside pytest and emit a machine-readable summary. A passing registry summary still reports `certification_effect = "none"`.

Run `python scripts/write_validation_status.py` to compose the source-backed benchmark registry, reference-target registry, and reference-adapter run reports into one machine-readable validation status report. The report is deliberately non-certifying: it records `certification_effect = "none"` and states that clinical and HTA reporting is disabled when no module holds Production Certified status. The same gates run in `.github/workflows/validation.yml`, which uploads the status JSON as a CI artifact.
...
| CI validation workflow | Dynamic GitHub Actions run | `.github/workflows/validation.yml` plus uploaded `validation-status` artifact | Runs tests and machine-readable gates; a green workflow is execution evidence, not production certification |
