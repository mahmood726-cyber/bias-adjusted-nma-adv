...
| Registry result/protocol boundary | Static source-policy contract plus unit fixtures | `src/bias_nma_adv/evidence_sources.py`, `src/bias_nma_adv/ingestion.py`, `validation/grand_benchmark_plan.toml`, and `tests/test_evidence_sources.py` | Allows AACT/ClinicalTrials.gov and public numeric ICTRP/PACTR result rows as effect sources; protocol-only registry rows are rejected as model-ready effect evidence |
