...
Current local outlier-space capability: `leave_one_out_outlier_diagnostic(...)` runs bounded pairwise leave-one-out refits and reports how much each omitted study changes the pooled estimate. This is a deterministic screening tool only; it does not replace full GOSH subset enumeration, outlier-model fitting, or mature package visualization workflows.
