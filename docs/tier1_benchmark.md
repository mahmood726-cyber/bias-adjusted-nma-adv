...
The dose-response coverage check is `validation/dose_response_source_coverage.toml`. It records that no active source-backed dose-response benchmark data are currently registered from ClinicalTrials.gov, PubMed abstracts, or open-access papers. The dose-response lane remains a planned `MBNMAdose` reference target until dose amounts, units, routes/frequencies/durations or equivalence fields, source-backed effects, source checks, and a dose-response analysis artifact are added.
...
Current dose-response data coverage: `dose_response_source_coverage.toml` reports `missing_source_backed_dose_response_data`. The repository has a planned `dose_response_mbnmadose` reference target, but no registered real-data dose-response benchmark artifact yet. Therefore there is currently no source-backed dose-response validation data from the allowed evidence-source boundary.
...
| Dose-response source coverage | Static source-coverage check plus validator | `validation/dose_response_source_coverage.toml`, `src/bias_nma_adv/dose_response_coverage.py`, and `tests/test_dose_response_coverage.py` | Records that no source-backed dose-response benchmark data are currently registered from ClinicalTrials.gov, PubMed abstracts, or open-access papers; blocks dose-response validation claims |
