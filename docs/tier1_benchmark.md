...
The DTA coverage check is `validation/dta_source_coverage.toml`. It records that no source-backed diagnostic test accuracy 2x2 benchmark data are currently registered and that no bivariate GLMM/HSROC DTA model is implemented.
...
| Diagnostic test accuracy | `mada`, `metafor`, or validated bivariate/HSROC references | Source-backed TP/FP/FN/TN tables, bivariate random-effects sensitivity/specificity, HSROC, threshold effects | Planned target only; current gate records no DTA model or source-backed DTA data |
...
Current DTA coverage: `validation/dta_source_coverage.toml` reports `missing_source_backed_dta_data` and `model_status = "not_implemented"`. DTA is feasible under the allowed source policy only when TP/FP/FN/TN tables, index-test labels, reference standards, and thresholds are source-bound to CT.gov, PubMed abstracts, or open-access papers. Until then, files named "diagnostics" remain statistical diagnostics, not DTA meta-analysis.
...
| DTA source coverage gate | Static source/model coverage check plus validator | `validation/dta_source_coverage.toml`, `src/bias_nma_adv/dta_coverage.py`, and `tests/test_dta_coverage.py` | Records that no source-backed DTA 2x2 benchmark data or bivariate/HSROC model is currently implemented; blocks DTA validation claims |
