...
- `src/bias_nma_adv/mlnmr_coverage.py`: Fail-closed ML-NMR coverage gate requiring public trial IPD, source-bound aggregate covariate distributions, overlap diagnostics, and `multinma` reference output before ML-NMR claims; simulated, synthetic, pseudo-IPD, survey-only, and closed patient datasets are excluded from real-data validation.
...
- `validation/simulation_full_report.json`: Executed non-certifying full simulation report. It currently contributes 25 full simulation jobs and 10,588 successful iterations to the large-scale gate, but it is not real clinical validation or superiority evidence.
...
- `validation/mlnmr_source_coverage.toml`: Machine-readable ML-NMR coverage gate. Current status is `missing_source_backed_mlnmr_data`; local portfolio survey/pseudo-IPD material and simulated `multinma` examples do not satisfy the real ML-NMR source boundary.
