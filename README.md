...
- `src/bias_nma_adv/multiarm.py`: Experimental contrast-level NMA solver preserving multi-arm covariance for netmeta-style parity tests, with pre-fit design diagnostics plus diagnostic-only GLS leverage, residual, approximate Cook-distance, absolute mapping-contribution, and fail-closed covariance-validity outputs.
