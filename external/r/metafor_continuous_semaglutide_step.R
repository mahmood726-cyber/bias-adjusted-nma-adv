...
parse_args <- function(args) {
  ...
  if (is.null(out$effects) || is.null(out$output) || is.null(out$benchmark_id)) {
    stop("Usage: metafor_continuous_semaglutide_step.R --benchmark-id <id> --effects <effects.csv> --output <output.json>")
  }
  out
}
...
main <- function() {
  ...
  if (any(effects$comparison != "semaglutide_2_4_mg_vs_placebo")) {
  ...
  output <- list(
    ...
    contrast = "semaglutide_2_4_mg_vs_placebo",
    ...
    limitations = list(
      "CT.gov adjusted treatment differences are used as reported; raw continuous IPD is not available.",
      "Different STEP populations are pooled only for software validation, not for clinical guidance.",
      "This validates one continuous mean-difference reference candidate only; it is not broad pairwise feature parity.",
