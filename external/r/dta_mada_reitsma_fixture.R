...
parse_args <- function(args) {
  out <- list(input = NULL, output = NULL, fixture_id = "dta_algorithmic_fixture")
  i <- 1
  while (i <= length(args)) {
    key <- args[[i]]
    if (key %in% c("--input", "--output", "--fixture-id")) {
      if (i + 1 > length(args)) {
      ...
      if (key == "--fixture-id") out$fixture_id <- val
      i <- i + 2
    } else {
      i <- i + 1
    }
  ...
  if (is.null(out$input) || is.null(out$output)) {
    stop("Usage: dta_mada_reitsma_fixture.R --input <input.csv> --output <output.json> [--fixture-id id]")
  }
  out
}

required_columns <- c("study_id", "tp", "fp", "fn", "tn")
...
main <- function() {
  ...
  rows <- utils::read.csv(parsed$input, stringsAsFactors = FALSE)
  missing <- setdiff(required_columns, names(rows))
  if (length(missing) > 0) {
    stop(paste("DTA fixture missing required columns:", paste(missing, collapse = ", ")))
  }
  if (nrow(rows) < 5) {
    stop("DTA fixture requires at least five studies for bivariate validation.")
  }
  ...
  output <- list(
    schema_version = "dta_mada_reitsma_fixture/v1",
    fixture_id = parsed$fixture_id,
    source_policy = "synthetic_algorithmic_fixture_not_clinical_evidence",
    ...
    warnings = list(
      "Algorithmic DTA fixture only; not source-backed clinical evidence.",
      "This reference output does not satisfy source-backed DTA validation requirements."
    )
  )
  jsonlite::write_json(
    output,
